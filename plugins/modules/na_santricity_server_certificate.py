...
options:
  ...
  certificates:
    description:
      - List of all server certificate files (Order does not matter).
      - Be aware that when no certificates are provided then a self-signed certificate generated by the storage system will be expected.
      - Certificates files can include root/intermediate authorities, public certificate, private key, and any combination in a certificate bundle.
      - Root/intermediate authority or the public server certificate files can be in either PEM or DER.
      - Private key file must be PEM format.
    type: list
    required: false
notes:
...
class NetAppESeriesServerCertificate(NetAppESeriesModule):
        ...
        try:
            ...
            with open(path, "r") as fh:
                ...
                while line != "":
                    ...
                    if re.search("^-+BEGIN CERTIFICATE-+$", line):
                        ...
                        certificates_info.update(self.certificate_info(info, certificate))
            ...
            try:
                with open(path, "rb") as fh:
                    ...
                    certificates_info.update(self.certificate_info(cert_info, cert_data))
                ...
                try:
                    with open(path, "rb") as fh:
                        loaded_privatekey = crypto.load_privatekey(crypto.FILETYPE_ASN1, fh.read())
                        pkey_info = loaded_privatekey.to_cryptography_key()
                        pkey_data = crypto.dump_privatekey(crypto.FILETYPE_PEM, loaded_privatekey)
                        certificates_info.update({"private_key": pkey_data})
                except Exception as error:
    ...
    def certificate_info(self, info, data):
    ...
    def get_expected_certificates(self):
        ...
        while certificates_info:
            ...
            else:  # If there are any here then the certificate was not issued by another. Just place them at the beginning. These will probably be root certs.
                for certificate_subject in certificates_info.keys():
                    ordered_certificates_info[ordered_certificates_info_index] = certificates_info[certificate_subject]
                    ordered_certificates_info_index -= 1
                break
