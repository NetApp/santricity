...
options:
  ...
  private_key_password:
    description:
      - Password for an encrypted I(private_key).
    type: str
    required: false
...
class NetAppESeriesServerCertificate(NetAppESeriesModule):
    ...
    def __init__(self):
        ...
        self.private_key_password = args["private_key_password"] if "private_key_password" in args.keys() else None
        ...
        self.der_encrypted_content = False
        ...
        try:
            with open(path, "r") as fh:
                ...
                while line != "":
                    ...
                    # Add private key to self.private_key_file_content.
            ...
            try:
                # Check for DER encrypted file.
                with open(path, "rb") as fh:
                    certificate = fh.read()
                    info = x509.load_der_x509_certificate(certificate, default_backend())
                    certificates_info.update(self.certificate_info(info, certificate))
            except Exception as error:
                self.module.fail_json(msg="Invalid certificate! Path [%s]. Array [%s]" % (path, self.ssid))
        except Exception as error:
    ...
    def get_effected_certificates(self):
        ...
        while certificates_info:
            for certificate_subject in certificates_info.keys():
                ...
                if certificate_subject not in [cert["issuer"] for cert in list(certificates_info.values()) + list(existing_certificates.values())]:
        ...
        # Check whether a certificate bundle included a private key.
        if self.private_key_file_content is not None:
            effected["private_key"] = self.private_key_file_content if six.PY2 else six.b(self.private_key_file_content)

        if self.private_key is not None:
            with open(self.private_key, "r" if six.PY2 else "rb") as fh:
                key = fh.read()

            # Verify whether multiple private keys were provided, if so, do they match; if they don't then fail.
