...
class NvmeInterfaceTest(ModuleTestCase):
    ...
    def test_get_interfaces_pass(self):
        ...
        with self._set_args({"address": "192.168.100.100"}):
            iface = NetAppESeriesIbIserInterface()
            ...
            with self.assertRaisesRegex(AnsibleFailJson, "Failed to detect any InfiniBand iSER interfaces!"):
                with mock.patch(
                    ...
                    return_value=(200, [
                        {
                            "interfaceType": "eth",
                            "eth": {"interfaceData": {"type": "ethernet", "infinibandData": {"isIser": False}}}
                        },
                        {
                            "interfaceType": "iscsi",
                            "iscsi": {"interfaceData": {"type": "infiniband", "infinibandData": {"isIser": False}}}
                        },
                        {
                            "interfaceType": "fc", "fc": {}
                        }
