...
class HostTest(ModuleTestCase):
    ...
    def test_host_exists_pass(self):
        ...
        with mock.patch(self.REQ_FUNC, return_value=(200, self.EXISTING_HOSTS)):
            with self._set_args({'state': 'present', 'name': 'new_host', 'host_type': 'linux dm-mp', 'force_port': False,
                                 'ports': [{'label': 'new_host_port_1', 'type': 'fc', 'port': '0x08ef08ef08ef08ef'}]}):
                host = NetAppESeriesHost()
                self.assertFalse(host.host_exists)

            with self._set_args({'state': 'present', 'name': 'does_not_exist', 'host_type': 'linux dm-mp',
                                 'ports': [{'label': 'beegfs_storage1_iscsi_0', 'type': 'iscsi',
                                            'port': 'iqn.1993-08.org.debian.beegfs-storage1:01:b0621126818'}]}):
                host = NetAppESeriesHost()
                self.assertFalse(host.host_exists)

            with self._set_args({'state': 'present', 'name': 'beegfs_storage1', 'host_type': 'linux dm-mp',
                                 'ports': [{'label': 'beegfs_storage1_iscsi_0', 'type': 'iscsi', 'port': 'iqn.differentiqn.org'}]}):
                host = NetAppESeriesHost()
                self.assertTrue(host.host_exists)

            with mock.patch(self.REQ_FUNC, return_value=(200, self.EXISTING_HOSTS)):
                with self._set_args({'state': 'present', 'name': 'beegfs_metadata1', 'host_type': 'linux dm-mp', 'force_port': True,
    ...
    def test_host_exists_fail(self):
        ...
        with self._set_args({'state': 'present', 'host_type': 'linux dm-mp', 'ports': [{'label': 'abc', 'type': 'iscsi', 'port': 'iqn:0'}]}):
            host = NetAppESeriesHost()
    ...
    def test_needs_update_pass(self):
        ...
        # No changes
        with mock.patch(self.REQ_FUNC, return_value=(200, self.EXISTING_HOSTS)):
            with self._set_args({'state': 'present', 'name': 'beegfs_metadata1', 'host_type': 'linux dm-mp',
                                 'ports': [{'label': 'beegfs_metadata1_iscsi_0', 'type': 'iscsi',
        ...
        # Change host type
        with mock.patch(self.REQ_FUNC, return_value=(200, self.EXISTING_HOSTS)):
            with self._set_args({'state': 'present', 'name': 'beegfs_metadata1', 'host_type': 'windows', 'force_port': False,
        ...
        # Add port to host
        with mock.patch(self.REQ_FUNC, return_value=(200, self.EXISTING_HOSTS)):
            with self._set_args({'state': 'present', 'name': 'beegfs_metadata1', 'host_type': 'linux dm-mp', 'force_port': False,
                                 'ports': [{'label': 'beegfs_metadata1_iscsi_1', 'type': 'iscsi', 'port': 'iqn.not_used'}]}):
                host = NetAppESeriesHost()
                exists = host.host_exists
                self.assertTrue(host.needs_update)
    ...
    def test_valid_host_type_pass(self):
        ...
        with mock.patch(self.REQ_FUNC, return_value=(200, self.HOST_TYPES)):
            with self._set_args({'state': 'present', 'host_type': '0'}):
                host = NetAppESeriesHost()
                self.assertTrue(host.valid_host_type)
            with self._set_args({'state': 'present', 'host_type': '28'}):
                host = NetAppESeriesHost()
                self.assertTrue(host.valid_host_type)
            with self._set_args({'state': 'present', 'host_type': 'windows'}):
                host = NetAppESeriesHost()
                self.assertTrue(host.valid_host_type)
            with self._set_args({'state': 'present', 'host_type': 'linux dm-mp'}):
                host = NetAppESeriesHost()
                self.assertTrue(host.valid_host_type)
    ...
    def test_valid_host_type_fail(self):
        ...
        with self.assertRaisesRegex(AnsibleFailJson, "host_type must be either a host type name or host type index found integer the documentation"):
            with self._set_args({'state': 'present', 'host_type': 'non-host-type'}):
                host = NetAppESeriesHost()

        with mock.patch(self.REQ_FUNC, return_value=(200, self.HOST_TYPES)):
            with self.assertRaisesRegex(AnsibleFailJson, "There is no host type with index"):
                with self._set_args({'state': 'present', 'host_type': '4'}):
                    host = NetAppESeriesHost()
    ...
    def test_assigned_host_ports_fail(self):
        ...
        with self.assertRaisesRegex(AnsibleFailJson, "There are no host ports available OR there are not enough unassigned host ports"):
            with mock.patch(self.REQ_FUNC, side_effect=[(200, self.EXISTING_HOSTS)]):
                with self._set_args({'state': 'present', 'name': 'beegfs_metadata3', 'host_type': 'linux dm-mp', 'force_port': False,
                                     'ports': [{'label': 'beegfs_metadata2_iscsi_0', 'type': 'iscsi', 'port': 'iqn.used_elsewhere'}]}):
                    host = NetAppESeriesHost()
                    exists = host.host_exists
                    host.assigned_host_ports(apply_unassigning=True)
    ...
    def test_update_host_pass(self):
        ...
        with self.assertRaises(AnsibleExitJson):
            with mock.patch(self.REQ_FUNC, return_value=(200, self.EXISTING_HOSTS)):
                with self._set_args({'state': 'present', 'name': 'beegfs_metadata1', 'host_type': 'windows', 'force_port': True,
    ...
    def test_create_host_fail(self):
        ...
        with self.assertRaisesRegex(AnsibleExitJson, "Host already exists."):
            with mock.patch(self.REQ_FUNC, side_effect=[(200, self.EXISTING_HOSTS)]):
                with self._set_args({'state': 'present', 'name': 'beegfs_storage1', 'host_type': 'linux dm-mp', 'force_port': True,
                                     'ports': [{'label': 'beegfs_storage1_iscsi_0', 'type': 'iscsi',
                                                'port': 'iqn.1993-08.org.debian.beegfs-storage1:01:b0621126818'}]}):
                    host = NetAppESeriesHost()
                    host.assigned_host_ports = _assigned_host_ports
    ...
    def test_remove_host_pass(self):
        ...
        with mock.patch(self.REQ_FUNC, return_value=(200, None)):
            with self._set_args({'state': 'absent', 'name': 'beegfs_metadata1', 'host_type': 'linux dm-mp', 'force_port': False,
                                 'ports': [{'label': 'beegfs_metadata1_iscsi_0', 'type': 'iscsi',
                                            'port': 'iqn.1993-08.org.debian.beegfs-metadata:01:69e4efdf30b8'}]}):
                host = NetAppESeriesHost()
