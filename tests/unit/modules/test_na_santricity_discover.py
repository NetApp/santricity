...
class AlertsTest(ModuleTestCase):
    ...
    def test_valid_options_fail(self):
        ...
        options_list = [{"ports": [0, 8443]}]

        for options in options_list:
            with self._set_args(options):
                with self.assertRaisesRegex(
                    AnsibleFailJson,
                    "Invalid port! Ports must be positive numbers between 0 and 65536."
                ):
                    discover = NetAppESeriesDiscover()
    ...
    def test_check_ip_address_pass(self):
        ...
        with self._set_args():
            with mock.patch(
                self.BASE_REQ_FUNC,
                return_value=(200, {"chassisSerialNumber": "012345678901", "storageArrayLabel": "array_label"})
            ):
                discover = NetAppESeriesDiscover()
                discover.check_ip_address(discover.systems_found, "192.168.1.100")

            self.assertEqual(
                discover.systems_found,
                {
                    "012345678901": {
                        "api_urls": ["https://192.168.1.100:8443/"],
                        "label": "array_label",
                        "addresses": [],
        ...
        for ports in [{"ports": [8080]}, {"ports": [8443]}]:
            with self._set_args(ports):
                port = ports["ports"][0]
                url_protocol = "https"
                if port == 8080:
                    url_protocol = "http"
                ...
                with mock.patch(
                    ...
                    side_effect=[
                        ...
                        (401, None),
                        (200, {"sa": {"saData": {
                            "chassisSerialNumber": "012345678901",
                ...
                self.assertEqual(
                    ...
                    {
                        "012345678901": {
                            "api_urls": [f"{url_protocol}://192.168.1.101:{port}/"],
                            "label": "array_label",
                            "addresses": [],
                            "proxy_required": False,
                            "proxy_ssid": ""
    ...
    def test_verify_proxy_service_pass(self):
        ...
        with self._set_args({"proxy_url": "https://192.168.1.200", "proxy_username": "admin", "proxy_password": "adminpass"}):
            discover = NetAppESeriesDiscover()
    ...
    def test_test_systems_found_pass(self):
        ...
        with self._set_args({
            "proxy_url": "https://192.168.1.200",
            "proxy_username": "admin",
            "proxy_password": "adminpass",
            "prefer_embedded": True
        }):
            discover = NetAppESeriesDiscover()
            with mock.patch(
                self.BASE_REQ_FUNC,
                return_value=(200, {"runningAsProxy": True})
            ):
                discover.test_systems_found(
                    discover.systems_found,
                    "012345678901",
                    "array_label", ["192.168.1.100", "192.168.1.102"]
                )
            ...
            self.assertEqual(
                discover.systems_found,
                {
                    "012345678901": {
                        "api_urls": [
                            "https://192.168.1.100:8443/devmgr/v2/",
    ...
    def test_proxy_discover_pass(self):
        ...
        with self._set_args({"subnet_mask": "192.168.1.0/30", "proxy_url": "https://192.168.1.200", "proxy_username": "admin", "proxy_password": "adminpass"}):
            discover = NetAppESeriesDiscover()
            ...
            with mock.patch(self.SLEEP_FUNC, return_value=None):
                with mock.patch(
                    ...
                    side_effect=[
                        (200, {"requestId": "1"}),
                        (200, {"discoverProcessRunning": True}),
                        (200, {
                            "discoverProcessRunning": False,
                            "storageSystems": [{
                                "controllers": [{"ipAddresses": ["192.168.1.100", "192.168.1.102"]}],
                                "supportedManagementPorts": ["https"],
        ...
        with self._set_args({
            ...
            with mock.patch(
                ...
                side_effect=[
                    (200, {
                        "012345678901": {
                            "api_urls": ["https://192.168.1.200:8443/"],
                            "label": "array_label",
                            "addresses": [],
                            "proxy_required": True,
                            "proxy_ssid": "1"
