...
class AuthTest(ModuleTestCase):
    ...
    def test_minimum_password_length_change_required_pass(self):
        ...
        with self._set_args({"ssid": "Proxy", "user": "admin", "password": "adminpass", "minimum_password_length": 8}):
        ...
        with self._set_args({"ssid": "Proxy", "user": "admin", "password": "adminpass", "minimum_password_length": 7}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: True
            auth.is_embedded_available = lambda: False
            with mock.patch(self.REQ_FUNC, return_value=(200, {"adminPasswordSet": False, "minimumPasswordLength": 8})):
        ...
        with self._set_args({"ssid": "10", "user": "admin", "password": "adminpass", "minimum_password_length": 8}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: True
            auth.is_embedded_available = lambda: False
            with mock.patch(self.REQ_FUNC, return_value=(200, {"adminPasswordSet": False, "minimumPasswordLength": 8})):
        ...
        with self._set_args({"ssid": "10", "user": "admin", "password": "adminpass", "minimum_password_length": 7}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: True
            auth.is_embedded_available = lambda: True
            with mock.patch(self.REQ_FUNC, return_value=(200, {"adminPasswordSet": False, "minimumPasswordLength": 8})):
        ...
        with self._set_args({"ssid": "1", "user": "admin", "password": "adminpass", "minimum_password_length": 8}):
            auth = NetAppESeriesAuth()
        ...
        with self._set_args({"ssid": "1", "user": "admin", "password": "adminpass", "minimum_password_length": 7}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: False
            auth.is_embedded_available = lambda: True
            with mock.patch(self.REQ_FUNC, return_value=(200, {"adminPasswordSet": False, "minimumPasswordLength": 8})):
    ...
    def test_minimum_password_length_change_required_fail(self):
        ...
        with self._set_args({"ssid": "Proxy", "user": "admin", "password": "adminpass", "minimum_password_length": 10}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: False
        ...
        with self._set_args({"ssid": "Proxy", "user": "admin", "password": "adminpass"}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: False
            with self.assertRaisesRegex(AnsibleFailJson, "Password does not meet the length requirement"):
    ...
    def test_password_change_required_pass(self):
        ...
        with self._set_args({"ssid": "Proxy", "user": "admin"}):
            auth = NetAppESeriesAuth()
        ...
        with self._set_args({"ssid": "10", "user": "admin", "password": "adminpass"}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: True
            auth.is_embedded_available = lambda: True
            with mock.patch(self.REQ_FUNC, side_effect=[(200, {"minimumPasswordLength": 8, "adminPasswordSet": False})]):
    ...
    def test_password_change_required_fail(self):
        ...
        with self._set_args({"ssid": "10", "user": "monitor", "password": "monitorpass"}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: True
            auth.is_embedded_available = lambda: False
            auth.logout_system = lambda: None
            ...
            with self.assertRaisesRegex(
                AnsibleFailJson,
                "Role based login not available! Only storage system password can be set for storage systems prior to"
            ):
    ...
    def test_set_array_admin_password_pass(self):
        ...
        with self._set_args({"ssid": "1", "user": "admin", "password": "adminpass"}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: False
            auth.is_embedded_available = lambda: True
            with mock.patch(self.REQ_FUNC, return_value=(200, None)):
    ...
    def test_set_array_password_fail(self):
        ...
        with self._set_args({"ssid": "Proxy", "user": "monitor", "password": "adminpass"}):
            auth = NetAppESeriesAuth()
            auth.is_proxy = lambda: True
            auth.is_embedded_available = lambda: False
            auth.is_admin_password_set = False
    ...
    def test_apply_pass(self):
        ...
        with self._set_args({"ssid": "1", "user": "monitor", "password": "adminpass"}):
            auth = NetAppESeriesAuth()
            auth.is_admin_password_set = True
            auth.password_change_required = lambda: True
            auth.minimum_password_length_change_required = lambda: True
